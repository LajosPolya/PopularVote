...
const CreatePolicy: React.FC<CreatePolicyProps> = ({
  ...
  const [fetchingPoliticians, setFetchingPoliticians] =
  ...
  useEffect(() => {
    const fetchPoliticians = async () => {
      ...
      try {
        ...
        const queryParams = levelOfPoliticsId
          ? `?levelOfPolitics=${levelOfPoliticsId}`
          : "";
        const response = await fetch(
          `${popularVoteApiUrl}/citizens/politicians${queryParams}`,
          {
            headers: {
      ...
      <Paper elevation={3} sx={{ p: 4, maxWidth: 600, mx: "auto" }}>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3 }}>
          ...
          <FormControl fullWidth sx={{ mb: 3 }}>
            <InputLabel id="co-authors-label">
              Co-Authors (Politicians)
            </InputLabel>
            <Select
              ...
              onChange={(e) =>
                setCoAuthorCitizenIds(e.target.value as number[])
              }
              ...
              {fetchingPoliticians ? (
                ...
                politicians.map((politician) => (
                  ...
                  >
                    ...
                    {(() => {
                      const party = politician.politicalAffiliationId
                        ? politicalParties.get(
                            politician.politicalAffiliationId,
                          )
                        : null;
