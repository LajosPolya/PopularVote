...
const PolicyDetails: React.FC<PolicyDetailsProps> = ({
  ...
  const [likedOpinionIds, setLikedOpinionIds] = useState<Set<number>>(
    new Set(),
  );
  ...
  const checkIsBookmarked = async () => {
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/policies/${policyId}/is-bookmarked`,
        { headers },
      );
      if (response.ok) {
  ...
  const checkHasVoted = async () => {
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/votes/policies/${policyId}/has-voted`,
        { headers },
      );
      if (response.ok) {
  ...
  const fetchLikedOpinions = async () => {
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/citizens/self/liked-opinions`,
        { headers },
      );
      if (response.ok) {
  ...
  const fetchLikeCounts = async (opinionIds: number[]) => {
    ...
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/opinions/likes/count?${queryParams}`,
        { headers },
      );
      if (response.ok) {
  ...
  const fetchPolicyDetails = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/policies/${policyId}/details`,
        { headers },
      );
  ...
  const handleBookmark = async () => {
    ...
    try {
      const token = await getAccessTokenSilently();
      const response = await fetch(
        `${popularVoteApiUrl}/policies/${policyId}/bookmark`,
        {
      ...
      if (response.ok) {
        ...
        console.error(
          `Failed to ${isBookmarked ? "unbookmark" : "bookmark"} policy`,
        );
      }
    } catch (err) {
      console.error(
        `Error ${isBookmarked ? "unbookmarking" : "bookmarking"} policy:`,
        err,
      );
    } finally {
  ...
  const handleToggleLike = async (opinionId: number) => {
    ...
    try {
      const token = await getAccessTokenSilently();
      const response = await fetch(
        `${popularVoteApiUrl}/opinions/${opinionId}/like`,
        {
    ...
    <Box sx={{ maxWidth: 900, mx: "auto" }}>
      ...
      <Paper elevation={3} sx={{ p: 4, mb: 4, position: "relative" }}>
        ...
        >
          ...
          {(() => {
            const party = policy.publisherPoliticalAffiliationId
              ? politicalParties.get(policy.publisherPoliticalAffiliationId)
              : null;
      ...
      >
        ...
        {!canWriteVotes && (
          <Alert
            ...
            onClick={onVerifyIdentity}
          >
            Verify your identity to unlock the ability to vote
          </Alert>
        )}
          ...
          {policy.opinions.map((opinion) => (
            ...
            >
              ...
              >
                ...
                <Tooltip
                  title={likedOpinionIds.has(opinion.id) ? "Unlike" : "Like"}
                >
                  <IconButton
                    ...
                    size="small"
                  >
                    {likedOpinionIds.has(opinion.id) ? (
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  {(() => {
                    const party = opinion.authorPoliticalAffiliationId
