...
const PoliticalPartyDetails: React.FC<PoliticalPartyDetailsProps> = ({
  ...
  const fetchPartyDetails = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/political-parties/${partyId}`,
        {
          headers: {
  ...
  const fetchPartyMembers = async () => {
    ...
    try {
      const token = await getAccessTokenSilently();
      const response = await fetch(
        `${popularVoteApiUrl}/political-parties/${partyId}/members`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchPartyPolicies = async () => {
    ...
    try {
      const token = await getAccessTokenSilently();
      const response = await fetch(
        `${popularVoteApiUrl}/political-parties/${partyId}/policies`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
                ...
                {members.map((member, index) => (
                  <React.Fragment key={member.id}>
                    ...
                    <ListItem disablePadding>
                      <ListItemButton
                        onClick={() =>
                          onCitizenClick && onCitizenClick(member.id)
                        }
                      >
                        <Box sx={{ mr: 2 }}>
                ...
                {policies.map((policy, index) => (
                  <React.Fragment key={policy.id}>
                    ...
                    <ListItem disablePadding>
                      <ListItemButton
                        onClick={() =>
                          onPolicyClick && onPolicyClick(policy.id)
                        }
                      >
                        <Box sx={{ mr: 2 }}>
