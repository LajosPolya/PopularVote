...
const PoliticianDeclaration: React.FC<PoliticianDeclarationProps> = ({
  onSuccess,
  onCancel,
}) => {
  ...
  const electoralDistrictsByProvince = useMemo(() => {
    if (!geoData)
      return new Map<
        number,
        { provinceName: string; districts: ElectoralDistrict[] }
      >();
    ...
    geoData.provincesAndTerritories.forEach((province) => {
      if (
        province.electoralDistricts &&
        province.electoralDistricts.length > 0
      ) {
        provinceMap.set(province.id, {
          provinceName: province.name,
          districts: [...province.electoralDistricts].sort((a, b) =>
  ...
  useEffect(() => {
    const fetchParties = async () => {
      ...
      try {
        ...
        const partiesByLevelOfPolitics: Map<number, PoliticalParty[]> =
          new Map();
        data.forEach((party) =>
          partiesByLevelOfPolitics.set(party.levelOfPoliticsId, [
            ...(partiesByLevelOfPolitics.get(party.levelOfPoliticsId) || []),
            party,
          ]),
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/citizens/self/declare-politician`,
        {
          method: "POST",
          headers: {
        ...
        <Box component="form" onSubmit={handleSubmit}>
          <Stack spacing={3}>
            ...
            <FormControl fullWidth required>
              ...
              >
                {Array.from(electoralDistrictsByProvince.entries()).map(
                  ([id, { provinceName }]) => (
            ...
            <FormControl fullWidth required disabled={provinceId === ""}>
              ...
              >
                {provinceId !== "" &&
                  electoralDistrictsByProvince
                    .get(provinceId as number)
            ...
            <FormControl fullWidth required>
              <InputLabel id="political-affiliation-label">
