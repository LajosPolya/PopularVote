...
import { CitizenProfile, CitizenSelf, Policy, PoliticalParty } from "./types";
...
const Profile: React.FC<ProfileProps> = ({
  ...
  const [citizen, setCitizen] = useState<CitizenProfile | CitizenSelf | null>(
    null,
  );
  ...
  const fetchCitizen = async () => {
    ...
    try {
      ...
      const endpoint = citizenId
        ? `${popularVoteApiUrl}/citizens/${citizenId}`
        : `${popularVoteApiUrl}/citizens/self`;
  ...
  const fetchPolicies = async (id: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/citizens/${id}/policies`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <Box sx={{ maxWidth: 800, mx: "auto" }}>
      ...
      <Paper elevation={3} sx={{ p: 4 }}>
        <Box sx={{ display: "flex", alignItems: "center", mb: 4 }}>
          ...
          <Box>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              ...
              />
              {(() => {
                const party = citizen.politicalAffiliationId
                  ? politicalParties.get(citizen.politicalAffiliationId)
                  : null;
        ...
        {!citizenId &&
          citizen &&
          "isVerificationPending" in citizen &&
          citizen.role === "CITIZEN" && (
            <Box sx={{ mt: 4, display: "flex", justifyContent: "center" }}>
              ...
              >
                {citizen.isVerificationPending
                  ? "Verification Pending"
                  : "Get Verified as a Politician"}
              </Button>
            </Box>
          )}
      </Paper>
