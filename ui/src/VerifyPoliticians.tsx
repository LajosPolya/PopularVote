...
const VerifyPoliticians: React.FC<VerifyPoliticiansProps> = ({
  onCitizenClick,
  politicalParties,
}) => {
  ...
  const fetchVerifications = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${popularVoteApiUrl}/citizens/verify-politician`,
        {
          headers: {
  ...
  const handleVerify = async (e: React.MouseEvent, id: number) => {
    ...
    try {
      const token = await getAccessTokenSilently();
      const response = await fetch(
        `${popularVoteApiUrl}/citizens/${id}/verify-politician`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
                          ...
                          {(() => {
                            const party = citizen.politicalAffiliationId
                              ? politicalParties.get(
                                  citizen.politicalAffiliationId,
                                )
                              : null;
